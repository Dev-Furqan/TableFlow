  ...
  return <div className="space-y-7">
    ...
    <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
      {riders.map((r:any)=> {
        ...
        const isAvailable=r.status!=='inactive'&&!jobs.length;
        return <Card key={r._id} className={`overflow-hidden p-5 ${isAvailable?'border-emerald-500/30':jobs.length?'border-amber-500/30':'border-rose-500/30'}`}>
          <div className="flex items-start justify-between gap-4">
            ...
            <Badge tone={isAvailable?'available':jobs.length?'pending':'inactive'}>{isAvailable?'Available':jobs.length?'On delivery':'Unavailable'}</Badge>
          </div>
